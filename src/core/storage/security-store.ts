...
export async function storeRecoveryPhrase(phrase: string): Promise<void> {
  // Simple base64 encoding - in production you'd want proper encryption
  const encoded = btoa(phrase);
  await chrome.storage.local.set({ [STORAGE_KEYS.RECOVERY_PHRASE_ENCRYPTED]: encoded });
}

/**
 * Get recovery phrase (for display in settings)
 */
export async function getRecoveryPhrase(): Promise<string | null> {
  ...
  const encoded = result[STORAGE_KEYS.RECOVERY_PHRASE_ENCRYPTED];
